...
# Aussi diagnostic pour l'installation de WhisperX et autres moteurs STT
...
check_problematic_dependencies() {
    ...
    if python -c "import whisperx" 2>/dev/null; then
        print_message "success" "whisperx est installé et importable"
    else
        print_message "warning" "whisperx n'est pas installé ou importable"
    fi
}
...
check_speech_recognition_engines() {
    ...
    if python -c "import whisperx" 2>/dev/null; then
        ...
        print_message "success" "WhisperX est disponible"
    fi
    ...
    if [[ "$engines_available" == false ]]; then
        ...
        print_message "info" "  - WhisperX: pip install whisperx torch"
...
check_python_version() {
    ...
    if [ "$PYTHON_MAJOR" = "3" ] && [ "$PYTHON_MINOR" = "10" ]; then
        ...
        warning "Cette application nécessite Python 3.10 spécifiquement pour la compatibilité avec PyTorch et WhisperX."
...
# Fonction pour vérifier l'installation de WhisperX
check_whisperx() {
    info "Vérification de l'installation de WhisperX..."
    
    # Vérifier si le module Python whisperx est importable
    if python_module_exists whisperx; then
        success "Le module Python 'whisperx' est importable"
        WHISPERX_MODULE_PATH=$(get_python_module_path whisperx)
        success "Chemin du module whisperx: $WHISPERX_MODULE_PATH"
        
        # Vérifier si torch est importable (dépendance de WhisperX)
        if python_module_exists torch; then
            ...
            error "Le module Python 'torch' n'est pas importable (dépendance de WhisperX)"
        fi
        
        # Vérifier si le modèle base peut être chargé
        info "Tentative de chargement du modèle WhisperX 'base'..."
        if python -c "import whisperx; whisperx.load_model('base', device='cpu', compute_type='int8')" >/dev/null 2>&1; then
            success "Le modèle WhisperX 'base' a été chargé avec succès"
        else
            error "Impossible de charger le modèle WhisperX 'base'"
            info "Cela peut être dû à un problème de téléchargement ou de permissions"
        fi
    else
        error "Le module Python 'whisperx' n'est pas importable"
        info "Solution: Installez WhisperX avec:"
        echo ""
        echo "pip install whisperx torch"
        echo ""
    fi
}
...
check_stt_engines_file() {
    ...
    if [ -f "$STT_ENGINES_FILE" ]; then
        ...
        if [ "$WHISPER_INSTALLED" != "$WHISPER_FILE" ] || [ "$VOSK_INSTALLED" != "$VOSK_FILE" ]; then
            info "Solution: Mettez à jour le fichier d'état avec:"
            echo ""
            echo "echo '{\"whisper\": $WHISPER_INSTALLED, \"vosk\": $VOSK_INSTALLED}' > $STT_ENGINES_FILE"
            echo ""
        fi
    else
...
generate_report() {
    ...
    echo "- WhisperX: $(python_module_exists whisperx && echo "Disponible" || echo "Non disponible")"
    ...
    if ! python_module_exists whisperx && ! python_module_exists vosk; then
