...
class RealtimeVoiceInterface:
    ...
    def _init_components(self):
        ...
        try:
            # Configuration WhisperX avec modèle large pour précision maximale
            stt_config = {
                'stt_settings': {
                    'engines': {
                        'whisperx': {
                            'enabled': True,
                            'model_name': 'large-v3',  # Modèle le plus précis (1550 MB)
                            'language': 'fr',
                            'priority': 1,
                            'parameters': {
                                'batch_size': 1,    # Batch minimal pour modèle très gros
                                'task': 'transcribe',
                                'language': 'french'
                            }
                        }
